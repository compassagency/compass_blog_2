...
$("#upload-avatar").on("change", function() {
	...
	reader.onload = function(e) {
		...
		$(".avatar-upload__image").append(img);
	};
...
$("#upload-image").on("change", function() {
	...
	reader.onload = function(e) {
		const img = $("<img>").attr("src", e.target.result);
		$(".comment-form__upload-image").empty().append(img);
	};
...
document.addEventListener('DOMContentLoaded', function () {
	...
	const commentsGrid = document.querySelector('#comments_component');

	commentForm.addEventListener('submit', function (event) {
		event.preventDefault();

		const avatarImageSrc = document.querySelector('.avatar-upload__image img') ? document.querySelector('.avatar-upload__image img').src : './assets/img/ava_default.png';
		const name = document.querySelector('input[name="author-name"]').value;
		const commentText = document.querySelector('textarea[name="comment"]').value;
		const uploadImageSrc = document.querySelector('.comment-form__upload-image img') ? document.querySelector('.comment-form__upload-image img').src : '';
		...
		newComment.innerHTML = `
				<div class="component_ava"><img src="${avatarImageSrc}" alt="${name}"></div>
				<div class="component_body">
					<div class="component_info">
						<div class="component_info_inner">
							...
							<div class="component_text"><p>${commentText}</p></div>
						</div>
					</div>
					<a href="#leadform" class="component_reposy">
						<b><span class="day-5">${formattedDate}</span></b>
						<nav>Like</nav>
						<nav>Reply</nav>
						<nav>More</nav>
					</a>
		...
		document.getElementById('new-comment').scrollIntoView({ behavior: 'smooth', block: 'start', inline: 'nearest', offsetTop: 100 });
	});
});
